"""
StyleFit AI Outfit Assistant - Databricks App
================================================
A Flask API that powers the AI-driven outfit recommendation system.
Deployed on Databricks Apps using OpenAI GPT-4o-mini and text-embedding-3-large.

Features:
1. Image Analysis - Analyze uploaded clothing images using GPT-4o-mini
2. RAG-based Matching - Find complementary items using text-embedding-3-large
3. Guardrails - Verify suggestions with visual AI comparison
4. Event-Based Styling - Recommend outfits for specific occasions
"""
...
# Model Configuration - OpenAI models
GPT_MODEL = "gpt-4o-mini"
EMBEDDING_MODEL = "text-embedding-3-large"

# Check if running on Databricks (for data path selection)
IS_DATABRICKS = bool(os.environ.get("DATABRICKS_HOST"))
...
# Data path - embedded in app deployment
# Data is included in the ./data directory alongside app.py
...
def get_embeddings(input_texts: List[str]):
    ...
    try:
        ...
        # Return zero vectors with correct dimension (3072 for text-embedding-3-large)
        return [[0.0] * 3072 for _ in input_texts]
...
def find_matching_items_with_rag(df_items, item_descs):
    ...
    for desc in item_descs:
        ...
        similar_indices = find_similar_items(input_embedding, embeddings, threshold=0.3, top_k=2)
...
"""
        ...
        return json.dumps({
            "items": ["Classic White T-shirt", "Blue Denim Jeans", "White Sneakers"],
            "category": "Casual",
            "gender": "Unisex",
...
Do not include the ```json``` tag."""
        ...
        print(f"⚠️ Match check error: {e}")
        return json.dumps({"match": True, "confidence": 70, "reason": "Style compatibility assumed"})
...
def get_event_outfit_recommendations(event_type: str, gender: str, style_preferences: str = ""):
    """
    Generate complete outfit recommendations for specific events.
    ...
    if df.empty:
        return {
            "outfit_items": ["Classic Blazer", "White Shirt", "Dark Trousers"],
            "style_tips": ["Keep it simple", "Match colors carefully"],
            "color_palette": "Neutral tones",
            "formality_level": "smart casual",
            "matched_inventory": []
        }
    
    # Step 1: Get AI recommendations using GPT-4o-mini
...
Keep outfit_items to exactly 3 items. Be specific with colors."""
    ...
    # Step 2: Find matching items from inventory
    print("   Step 2: Finding inventory matches...")
    matched_items = []
    outfit_items = recommendations.get('outfit_items', [])[:3]
    
    if outfit_items and not df.empty:
        ...
        if len(gender_filter) > 0:
            ...
            try:
                ...
                for i, (item_desc, embedding) in enumerate(zip(outfit_items, all_embeddings)):
                    similar = find_similar_items(embedding, embeddings_list, threshold=0.3, top_k=1)
...
def health_check():
    ...
    images_dir = os.path.join(DATA_PATH, 'sample_images')
    ...
    # Debug: Check various paths
    paths_to_check = [
        "/dbfs/retailnext-data",
        "/Volumes/sarbanimaiti_catalog/style-demo/style-vol", 
        "./data",
        os.getcwd(),
        "/app",
        "/Workspace",
    ...
    for path in paths_to_check:
        try:
            ...
            path_debug[path] = {"exists": exists}
            if exists and os.path.isdir(path):
                path_debug[path]["contents"] = os.listdir(path)[:5]
        except Exception as e:
            path_debug[path] = {"error": str(e)}
    ...
    return jsonify({
        "status": "healthy",
        "mode": "databricks" if IS_DATABRICKS else "openai",
        "llm_model": GPT_MODEL,
        "embedding_model": EMBEDDING_MODEL,
        "data_path": DATA_PATH,
        "data_path_exists": os.path.exists(DATA_PATH),
        "dataset_loaded": len(df) if not df.empty else 0,
...
def analyze_clothing():
    ...
    subcategories = df['articleType'].unique() if not df.empty else ['Casual', 'Formal']
    ...
    matching_items = find_matching_items_with_rag(filtered_df, item_descs) if not filtered_df.empty else []
...
def get_image(item_id):
    ...
    image_path = os.path.join(DATA_PATH, 'sample_images', f"{item_id}.jpg")
    ...
    if os.path.exists(image_path):
        ...
        return jsonify({"error": f"Image {item_id}.jpg not found at {image_path}"}), 404
...
def debug_images():
    ...
    if os.path.exists(images_dir):
        ...
        return jsonify({
            ...
            "count": len(images),
            "images": images[:20],
            "sample_ids": [int(f.replace('.jpg', '')) for f in images[:10]]
        })
...
def get_inventory():
    ...
    if df.empty:
        return jsonify({"total_items": 0, "categories": {}, "genders": {}, "sample_items": []})
...
def semantic_search():
    ...
    if df.empty:
        return jsonify({"results": [], "query": query, "debug": "Dataset is empty"})
    ...
    try:
        ...
        return jsonify({"results": [], "query": query, "debug": f"Embedding error: {str(e)}"})
    
    # Check embedding dimensions
    sample_embedding = filtered_df['embeddings'].iloc[0]
    data_dim = len(sample_embedding)
    ...
    return jsonify({
        ...
        "debug": {
            ...
            "data_embedding_dim": data_dim,
            "matches_found": len(similar),
            "embedding_error": _last_embedding_error
        }
    })
...
if __name__ == '__main__':
    ...
    print("🚀 StyleFit AI Outfit Assistant")
    print("=" * 50)
    print(f"Mode: {'Databricks App' if IS_DATABRICKS else 'Local Development'}")
    ...
    app.run(host='0.0.0.0', port=port, debug=not IS_DATABRICKS)
